...
class LoginController extends Controller {
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if (!$data = $login->create()) {
                ...
                header("Content-type: text/html; charset=utf-8");
                 $res = array(
                    'status' => false,
                    'info' => $login->getError(),
                    'callback' => U('Home/Index/Login/login')
                );
            ...
            if ($result && $result['password'] == $data['password']) {
                ...
                // 更新用户登录信息
                //$this->success('登录成功,正跳转至系统首页...', U('Home/Index/Index/index'));
                $res = array(
                    'status' => true,
                    'info' => 'ok',
                    'callback' => U('Home/Index/Index/index')
                );
                $this->ajaxReturn($res,'JSON');
            } else {
   ...
   /*  public function register()
    ...
    public function logout()
    {
        ...
        // 清楚所有session
        session(null);
        redirect(U('Home/Index/Login/login'), 0, '正在退出登录...');
    }
